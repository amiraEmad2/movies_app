class SimilarModel {
  ...
  SimilarModel.fromJson(Map<String, dynamic> json) {
    if(json["page"] is int) {
      ...
      page = json["status_code"];
    } if(json["status_message"] is int) {
      page = json["status_message"];
    }
    if(json["results"] is List) {
      results = json["results"] == null ? null : (json["results"] as List).map((e) => SimilarResults.fromJson(e)).toList();
    }
...
class SimilarResults {
  ...
  SimilarResults({this.adult, this.backdropPath, this.genreIds, this.id, this.originalLanguage, this.originalTitle, this.overview, this.popularity, this.posterPath, this.releaseDate, this.title, this.video, this.voteAverage, this.voteCount});
